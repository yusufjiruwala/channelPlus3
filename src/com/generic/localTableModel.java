...
public class localTableModel implements TableModel, Serializable {
	...
	public void setAdvanceColumnsFilter(List<Parameter> plstFilter)
			throws Exception {
	...
	public void setColumnsFilter2(List<Parameter> plstFilter) {
		...
		for (int i = 0; i < lstFilter.size(); i++) {
			if (lstFilter.get(i).getValue() != null
					&& lstFilter.get(i).getValue().toString().trim().length() > 0) {
				no_of_fnd++;
			}
		}
		if (no_of_fnd == 0) {
		...
		for (int i = 0; i < masterRows.size(); i++) {
			...
			for (int j = 0; j < lstFilter.size(); j++) {
				...
				fl = "";
				if (r.lst.get(
						getColByName(lstFilter.get(j).getName()).getColpos())
						.getValue() != null) {
					s = r.lst
							.get(getColByName(lstFilter.get(j).getName())
									.getColpos()).getValue().toString();
				}
				...
				// string
				if (!fl.startsWith("=")
						&& s.toUpperCase().contains(fl.toUpperCase())
						&& lstFilter.get(j).getValueType()
								.equals(Parameter.DATA_TYPE_STRING)
						&& fl.length() > 0) {
					fnd++;
				}
				if (fl.startsWith("=")
						&& s.toUpperCase().equals(fl.substring(1))
						&& lstFilter.get(j).getValueType()
								.equals(Parameter.DATA_TYPE_STRING)
						&& fl.length() > 0) {
					fnd++;
				...
				if (fl.length() > 0
					...
					if (fl.indexOf("=") == 0
							&& fl.length() > 0
							&& s.length() > 0
							&& Double.valueOf(fl.substring(1)).doubleValue() == Double
					...
					if (fl.indexOf(">=") == 0
							&& fl.length() > 0
							&& s.length() > 0
							&& Double.valueOf(fl.substring(2)).doubleValue() <= Double
									.valueOf(s).doubleValue()) {
						fnd++;
					}
			...
			for (int i = 0; i < masterRows.size(); i++) {
				...
				for (int j = 0; j < r.lst.size(); j++) {
					if (dynamicFilter == null
							...
							|| r.lst.get(j).getDisplay().toUpperCase()
									.contains(dynamicFilter.toUpperCase())) {
						fnd = true;
						break;
					}
				}
	...
	public Double getSummaryOf(List<String> listGroupsBy, String colname, int st) {
		...
		if (st == SUMMARY_SUM) {
			...
			if (getFieldValue(cursorNo, colname) != null) {
				vl = vl
						+ ((Number) getFieldValue(cursorNo, colname))
								.doubleValue();
			}
			...
			for (int i = cursorNo - 1; i >= 0; i--) {
				if (isOldRowDiff(listGroupsBy, mp, i)) {
					...
					if (getFieldValue(i, colname) != null) {
						vl = vl
								+ ((Number) getFieldValue(i, colname))
										.doubleValue();
					}
				}
			}
	...
	private void setgroupByRows(List<String> listGroupsBy,
	...
	private boolean isOldRowDiff(List<String> listGroupsBy,
			Map<String, Object> mp, int rown) {
		boolean fnd = false;
		for (int i = 0; i < listGroupsBy.size(); i++) {
	...
	public double getCountOf(String colname, Object val) {
		...
		for (int i = 0; i < rows.size(); i++) {
			...
			if (val instanceof String) {
				if (getFieldValue(i, colname).toString().toUpperCase()
						.equals(((String) val).toUpperCase()))
					cnt++;
			}
		}
		return cnt;
	...
	public DBClass createDBClassFromConnection(Connection c)
			throws SQLException {
	...
	public qryColumn getColByName(String name) {
		for (int i = 0; i < qrycols.size(); i++) {
			if (qrycols.get(i).getColname().trim().toUpperCase()
	...
	public void executeQuery(String sq, String[] visibleCol)
			throws SQLException {
	...
	public Object visibleColValue(Row r, int colIndex) {
		if (colIndex > visbleQrycols.size()
				|| visbleQrycols.get(colIndex) == null) {
			return null;
		}
	...
	public String getDisplayAt(int rowIndex, int columnIndex) {
		...
		if (visbleQrycols.get(columnIndex).getNumberFormat().length() > 0) {
			if (s != null && s instanceof dataCell
				...
				s = (new DecimalFormat(visbleQrycols.get(columnIndex)
						.getNumberFormat()).format(Double
						.valueOf(((dataCell) s).getValue().toString())));
			}
		}
	...
	public void setFieldValue(int rowno, String colname, Object value) {
		...
		rows.get(rowno).lst.get(getColByName(colname).getColpos()).setValue(
				utils.nvl(value, ""), value);
		if (rowlistner != null && rowsTriggerExecute == true) {
			try {
				rowsTriggerExecute = false;
	...
	public int insertRecord(int index) {
		...
		for (int i = 0; i < cols; i++) {
			r.lst.get(i).setValue(qrycols.get(i).getDefaultValue(),
					qrycols.get(i).getDefaultDisplay());
		}
		if (index == rows.size()) {
			rows.add(r);
		} else {
		...
		if (defaultValuelistner != null && rowsTriggerExecute == true) {
			try {
				...
				for (int i = 0; i < r.lst.size(); i++) {
					defaultValuelistner.setValue(r.lst.get(i),
							getVisbleQrycols().get(i));
				}
				if (rowlistner != null) {
					rowlistner.onCalc(cursorNo);
				}
