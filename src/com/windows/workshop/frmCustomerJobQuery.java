...
public class frmCustomerJobQuery implements transactionalForm {
	Connection con = null;
	...
	public void showInitView() {
		showOpenedJobVouchers();
	}

	public void showOpenedJobVouchers() {
		...
		try {
			...
			qv
					.setSqlquery("select keyfld,posted_user,posted_date_time Posted_Time,kind, action_type, user_descr1,long_data from approval_all where reaction='STAND_BY'"
			...
			pm
					.setLovsql("select distinct posted_user,posted_user u2 from approval_all where reaction='STAND_BY'");
				...
				public void afterVisual() {
					...
					qv.getTable().addListener(new ValueChangeListener() {
						...
						public void valueChange(ValueChangeEvent event) {
							...
							String s = qv.getLctb().getFieldValue(rowno,
									"LONG_DATA").toString();
							try {
			...
			qv.createView();
			layOut = new CustomLayout(new ByteArrayInputStream("<br><br>"
					.getBytes()));
				...
				public void buttonClick(ClickEvent event) {
					...
					try {
						...
						utils
								.execSql(
										"update approval_all set "
												+ "reaction='CANCELLED' , approved_date_time=sysdate,"
												+ "approved_user_by='"
												+ utils.CPUSER
												+ "'  where keyfld=" + keyid,
										con);
