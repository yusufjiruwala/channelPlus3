...
public class frmSaleReturn implements transactionalForm {
	...
	public void save_data(boolean cls) {
		...
		try {
			...
			// ---------------------- all database field's value in qe.setpara
			// --------------------- for saving ORDER1
						...
						.valueOf(QueryExe
								.getSqlValue(
										"select nvl(max(ord_price),0) "
												+ " from joined_order where ord_refer= :RFR and "
												+ " ord_rcptno= :RNO and ord_reference= :ON and ord_code=103 ",
										con,
										"0.0",
			...
			sal_return(Double.valueOf(txtNo.getValue() + ""));
	...
	private void sal_return(Double ordno) throws SQLException {
		if (chkFlag.booleanValue())
			utils.execSql("begin x_sal_return('" + ordno + "',false); end;",
					con);
		else
	...
	public void load_data() {
		try {
			...
			if (!QRYSES.isEmpty()) {
				...
				txtShipingDate.setEnabled(false);
	...
	public void applyColumns() {
		try {
			utilsVaadin.applyColumns("FRMSALRET.ORDER2", table.getTable(),
			...
			utilsVaadin.findColByCol("ORD_PRICE", table.listFields).actionAfterUpdate = new afterModelUpdated() {
				public Object onValueChange(int rowno, String colname, Object vl) {
					...
					double disc = ((Number) table.data.getFieldValue(rowno,
							"ORD_DISCAMT")).doubleValue();
					double pkqt = ((Number) table.data.getFieldValue(rowno,
							"ORD_PKQTY")).doubleValue();
					double qt = ((Number) table.data.getFieldValue(rowno,
							"ORD_UNQTY")).doubleValue();
					double pk = ((Number) table.data.getFieldValue(rowno,
							"ORD_PACK")).doubleValue();

					double allqt = (pkqt * pk) + qt;

					double pr = ((Number) table.data.getFieldValue(rowno,
	...
	private void show_selection_items(boolean show_only_empty_table)
		...
		tbl.FetchSql("select distinct items.reference cost_item,items.descr cost_item_descr,"
				...
				+ " 0 fc_price,1 fc_rate,'KWD' fc_descr,o2.payment_reference,(ord_allqty+saleret_qty)-(issued_qty+purret_qty) Qty_in_Hand from "
				+ " items ,order2 o2,order1 o1 "
				+ " where "
				+ " o2.ord_rcptno is not null  "
				+ " and o2.ord_refer=items.reference and saleret_qty=0 and issued_qty>0 "
				...
				try {
					for (int i = 0; i < tbl.getLstCheckSelection().size(); i++) {
						if (tbl.getLstCheckSelection().get(i).booleanValue()) {
							...
							// table.data.setFieldValue(lr, "ORD_COST_PRICE",
							// tbl
							...
							String rn = utils.nvl(
									tbl.getData().getFieldValue(i, "RCPT_NO"),
									"");
							String on = utils.nvl(
									tbl.getData().getFieldValue(i, "ORD_NO"),
									"");

							ResultSet rst = QueryExe.getSqlRS(
									"select ord_price*ord_fc_rate,ord_discamt*ord_fc_rate from order2 "
											+ " where ord_no='" + on
											+ "' and ord_rcptno='" + rn
							...
							if (rst != null) {
								rst.first();
								table.data.setFieldValue(lr, "ORD_PRICE",
										BigDecimal.valueOf(rst.getDouble(1)));
								table.data.setFieldValue(lr, "ORD_DISCAMT",
										BigDecimal.valueOf(rst.getDouble(2)));
								rst.close();
