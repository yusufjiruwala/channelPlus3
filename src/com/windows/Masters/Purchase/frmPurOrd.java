...
public class frmPurOrd implements transactionalForm {
	...
	private TextField txtSuppInv = ControlsFactory.CreateTextField(null,
			"ATTN", lstfldinfo);
	...
	private Label lblCust = new Label("Supplier");
	private Label lblDueDate = new Label("Due Date");
	private Label lblPayItem = new Label("Remarks");
	private Label lblSuppInv = new Label("Supp. Invoice Ref#");
	...
	private void save_parameters() throws SQLException {
		...
		Channelplus3Application.getInstance().getFrmUserLogin().getMapVars()
				...
				"select accno,name from acaccount where accno='"
						+ Channelplus3Application.getInstance()
								.getFrmUserLogin().getMapVars().get(
										"PURCHASE_ORDER_ACCOUNT") + "'", con);
		if (rso != null && rso.first()) {
			txtOrderAcc.setDisplayValue(rso.getString("ACCNO"), rso
					.getString("NAME"));
		}
	}
	...
	public void createView() {
		...
		ResourceManager.addComponent(attentionLayout, lblSuppInv);
		ResourceManager.addComponent(attentionLayout, txtSuppInv);
		...
		attentionLayout.setComponentAlignment(lblSuppInv,
		...
		attentionLayout.setExpandRatio(lblSuppInv, 1f);
		attentionLayout.setExpandRatio(txtSuppInv, 3f);
	...
	public void validateData() throws Exception {
		...
		if (txtCust.getValue() == null
			...
			throw new SQLException("Supplier must have value");
		...
		if (txtShimp.getValue() != null
			...
			if (!typ.equals("1"))
				sq = "select distinct reference,items.descr from items,order2 where order2.ord_code=decode("
						+ typ
						+ ",2,106,3,100) and ord_refer=reference and order2.ord_no='"
						+ txtShimp.getValue() + "'";
