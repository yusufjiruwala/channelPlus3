...
public class frmSale implements transactionalForm {
	...
	private VerticalLayout detailsBasicLayout = new VerticalLayout();
	...
	private HorizontalLayout detContent1Layout = new HorizontalLayout();
	private HorizontalLayout areaLayout = new HorizontalLayout();
	private HorizontalLayout blockLayout = new HorizontalLayout();

	private HorizontalLayout detContent2Layout = new HorizontalLayout();
	private HorizontalLayout jeddaLayout = new HorizontalLayout();
	private HorizontalLayout streetLayout = new HorizontalLayout();

	private HorizontalLayout detContent3Layout = new HorizontalLayout();
	private HorizontalLayout bldgLayout = new HorizontalLayout();
	...
	private HorizontalLayout driverLayout = new HorizontalLayout();
	...
	private Label lblBlock = new Label("Block");
	private Label lblJedda = new Label("Jedda");
	private Label lblStreet = new Label("Street");
	private Label lblBldg = new Label("Bldg");
	private Label lblTel = new Label("Tel");
	private Label lblKeyId = new Label("Key Id");
	private Label lblOrder = new Label("Ord No");
	private Label lblLocationCode = new Label("Loc Code");
	private Label lblDriver = new Label("Driver");
	private Label lblVehicle = new Label("Vehicle");
	private Label lblIssuedDate = new Label("Issued Date");
	...
	private TextField txtNetAmt = ControlsFactory.CreateTextField(null, "",
			null);
	private TextField txtArea = ControlsFactory.CreateTextField(null,
			"ADDR_AREA", lstfldinfo);
	private TextField txtBlock = ControlsFactory.CreateTextField(null,
			"ADDR_BLOCK", lstfldinfo);
	private TextField txtJedda = ControlsFactory.CreateTextField(null,
			"ADDR_JEDDA", lstfldinfo);
	private TextField txtStreet = ControlsFactory.CreateTextField(null,
			"ADDR_STREET", lstfldinfo);
	private TextField txtBldg = ControlsFactory.CreateTextField(null,
			"ADDR_BLDG", lstfldinfo);
	private TextField txtTel = ControlsFactory.CreateTextField(null,
			"ADDR_TEL", lstfldinfo);
	private TextField txtKeyId = ControlsFactory.CreateTextField(null,
			"KEYFLD", lstfldinfo);
	private TextField txtOrder = ControlsFactory
			.CreateTextField(null, "", null);
	private ComboBox txtLocationCode = ControlsFactory.CreateListField(null,
			...
			lstfldinfo);
	private TextField txtDriverNo = ControlsFactory.CreateTextField(null,
			"ORDNO", lstfldinfo);
	private TextField txtDriverName = ControlsFactory.CreateTextField(null, "",
			null);
	private TextField txtVehicle = ControlsFactory.CreateTextField(null, "",
			null);
	...
	private NativeButton cmdClearCommAcc = ControlsFactory.CreateToolbarButton(
			"img/clear.png", "Clear...");
	private NativeButton cmdDriverNo = ControlsFactory.CreateToolbarButton(
			"img/find.png", "Find...");
	private NativeButton cmdClearDriverName = ControlsFactory
			.CreateToolbarButton("img/clear.png", "Clear...");
	...
	public void load_data() {
		try {
			...
			utilsVaadin.resetValues(detailsBasicLayout, false, false);
			utilsVaadin.resetValues(otherInfoBasicLayout, false, false);
			utilsVaadin.resetValues(tableTextLayout, false, false);

			if (!QRYSES.isEmpty()) {
				PreparedStatement pstmt = con.prepareStatement(
						"select * from pur1 where invoice_code=21 and keyfld='"
								+ QRYSES + "'",
	...
	public void createView() {
		...
		detailsBasicLayout.setWidth("100%");
		...
		detContent1Layout.setWidth("100%");
		areaLayout.setWidth("100%");
		blockLayout.setWidth("100%");
		detContent2Layout.setWidth("100%");
		jeddaLayout.setWidth("100%");
		streetLayout.setWidth("100%");
		detContent3Layout.setWidth("100%");
		bldgLayout.setWidth("100%");
		...
		lblArea.setWidth("60px");
		lblBlock.setWidth("60px");
		lblJedda.setWidth("60px");
		lblStreet.setWidth("60px");
		lblBldg.setWidth("60px");
		lblTel.setWidth("60px");
		lblKeyId.setWidth("60px");
		lblOrder.setWidth("60px");
		lblLocationCode.setWidth("60px");
		lblDriver.setWidth("60px");
		lblVehicle.setWidth("60px");
		...
		txtBlock.setWidth("100%");
		txtJedda.setWidth("100%");
		txtStreet.setWidth("100%");
		txtBldg.setWidth("100%");
		txtTel.setWidth("100%");
		txtKeyId.setWidth("100%");
		txtOrder.setWidth("100%");
		txtLocationCode.setWidth("100%");
		txtDriverNo.setWidth("100%");
		txtDriverName.setWidth("100%");
		...
		tbsheet.addTab(detailsBasicLayout, "Details", null);
		tbsheet.addTab(otherInfoBasicLayout, "Other Info", null);

		salesBasicLayout.addStyleName("formLayout");
		tableTextLayout.addStyleName("formLayout");
		detailsBasicLayout.addStyleName("formLayout");
		otherInfoBasicLayout.addStyleName("formLayout");

		tbsheet.addTab(salesBasicLayout, "Basic Information", null);
		tbsheet.addTab(detailsBasicLayout, "Branches & A/C", null);
		...
		ResourceManager.addComponent(detailsBasicLayout, detContent1Layout);
		ResourceManager.addComponent(detailsBasicLayout, detContent2Layout);
		ResourceManager.addComponent(detailsBasicLayout, detContent3Layout);
		...
		ResourceManager.addComponent(detContent1Layout, areaLayout);
		ResourceManager.addComponent(detContent1Layout, blockLayout);

		ResourceManager.addComponent(detContent2Layout, jeddaLayout);
		ResourceManager.addComponent(detContent2Layout, streetLayout);

		ResourceManager.addComponent(detContent3Layout, bldgLayout);
		ResourceManager.addComponent(detContent3Layout, telLayout);

		ResourceManager.addComponent(otherInfoContent1Layout, keyLayout);
		ResourceManager.addComponent(otherInfoContent1Layout, orderLayout);
		ResourceManager.addComponent(otherInfoContent1Layout,
				locationCodeLayout);

		ResourceManager.addComponent(otherInfoContent2Layout, driverLayout);
		...
		ResourceManager.addComponent(blockLayout, lblBlock);
		ResourceManager.addComponent(blockLayout, txtBlock);

		ResourceManager.addComponent(jeddaLayout, lblJedda);
		ResourceManager.addComponent(jeddaLayout, txtJedda);

		ResourceManager.addComponent(streetLayout, lblStreet);
		ResourceManager.addComponent(streetLayout, txtStreet);

		ResourceManager.addComponent(bldgLayout, lblBldg);
		ResourceManager.addComponent(bldgLayout, txtBldg);
		...
		ResourceManager.addComponent(orderLayout, txtOrder);
		...
		ResourceManager.addComponent(driverLayout, lblDriver);
		ResourceManager.addComponent(driverLayout, txtDriverNo);
		ResourceManager.addComponent(driverLayout, cmdDriverNo);
		ResourceManager.addComponent(driverLayout, cmdClearDriverName);
		ResourceManager.addComponent(driverLayout, txtDriverName);
		...
		blockLayout.setComponentAlignment(lblBlock, Alignment.BOTTOM_CENTER);

		jeddaLayout.setComponentAlignment(lblJedda, Alignment.BOTTOM_CENTER);

		streetLayout.setComponentAlignment(lblStreet, Alignment.BOTTOM_CENTER);

		bldgLayout.setComponentAlignment(lblBldg, Alignment.BOTTOM_CENTER);
		...
		driverLayout.setComponentAlignment(lblDriver, Alignment.BOTTOM_CENTER);
		driverLayout
				.setComponentAlignment(cmdDriverNo, Alignment.BOTTOM_CENTER);
		driverLayout.setComponentAlignment(cmdClearDriverName,
				Alignment.BOTTOM_CENTER);
		...
		detContent1Layout.setExpandRatio(areaLayout, 2f);
		detContent1Layout.setExpandRatio(blockLayout, 2f);

		detContent2Layout.setExpandRatio(jeddaLayout, 2f);
		detContent2Layout.setExpandRatio(streetLayout, 2f);

		detContent3Layout.setExpandRatio(bldgLayout, 2f);
		detContent3Layout.setExpandRatio(telLayout, 2f);

		otherInfoContent1Layout.setExpandRatio(keyLayout, 1.3f);
		otherInfoContent1Layout.setExpandRatio(orderLayout, 1.3f);
		otherInfoContent1Layout.setExpandRatio(locationCodeLayout, 1.4f);

		otherInfoContent2Layout.setExpandRatio(driverLayout, 2.6f);
		...
		blockLayout.setExpandRatio(lblBlock, 0.1f);
		blockLayout.setExpandRatio(txtBlock, 3.9f);

		jeddaLayout.setExpandRatio(lblJedda, 0.1f);
		jeddaLayout.setExpandRatio(txtJedda, 3.9f);

		streetLayout.setExpandRatio(lblStreet, 0.1f);
		streetLayout.setExpandRatio(txtStreet, 3.9f);

		bldgLayout.setExpandRatio(lblBldg, 0.1f);
		bldgLayout.setExpandRatio(txtBldg, 3.9f);

		telLayout.setExpandRatio(lblTel, 0.1f);
		telLayout.setExpandRatio(txtTel, 3.9f);

		keyLayout.setExpandRatio(lblKeyId, 0.1f);
		keyLayout.setExpandRatio(txtKeyId, 3.9f);

		orderLayout.setExpandRatio(lblOrder, 0.1f);
		orderLayout.setExpandRatio(txtOrder, 3.9f);

		locationCodeLayout.setExpandRatio(lblLocationCode, 0.1f);
		locationCodeLayout.setExpandRatio(txtLocationCode, 3.9f);

		driverLayout.setExpandRatio(lblDriver, 0.1f);
		driverLayout.setExpandRatio(txtDriverNo, 1f);
		driverLayout.setExpandRatio(cmdDriverNo, 0.1f);
		driverLayout.setExpandRatio(cmdClearDriverName, 0.1f);
		driverLayout.setExpandRatio(txtDriverName, 2.7f);
		...
		if (!listnerAdded) {
			...
			cmdDriverNo.addListener(new ClickListener() {
				public void buttonClick(ClickEvent event) {
					show_DriverList();
				}
			});
			...
			cmdClearDriverName.addListener(new ClickListener() {
				public void buttonClick(ClickEvent event) {
					txtDriverNo.setReadOnly(false);
					txtDriverName.setReadOnly(false);
					txtDriverNo.setValue("");
					txtDriverName.setValue("");
					txtDriverNo.setReadOnly(true);
					txtDriverName.setReadOnly(true);
				}
			});
			...
			utilsVaadin
					.showSearch(
							...
							"select invoice_no,invoice_date,c_cus_no,inv_refnm cust_name,round(inv_amt-disc_amt,3)"
									+ " net_amount,keyfld from pur1 where invoice_code=21 order by invoice_date desc , invoice_no desc",
							true);
	...
	protected void show_DriverList() {
		try {
			final Window wnd = new Window();
			final VerticalLayout la = new VerticalLayout();
			wnd.setContent(la);
			...
			utilsVaadin.showSearch(la, new SelectionListener() {
				public void onSelection(TableView tv) {
					...
					if (tv.getSelectionValue() > -1) {
						try {
							int rn = tv.getSelectionValue();
							txtDriverName.setValue(tv.getData().getFieldValue(
									rn, "name").toString());
							txtDriverNo.setValue(tv.getData().getFieldValue(rn,
									"no").toString());
						} catch (Exception ex) {
							ex.printStackTrace();
						}
			...
			}, con, "Select no,name from salesp where type='D' order by no",
					true);
		} catch (SQLException ex) {
			ex.printStackTrace();
