...
public class frmSaleOrd implements transactionalForm {
	...
	public void save_data(boolean cls) {
		...
		try {
			...
			qe.setParaValue("ORD_AMT", df.parse(utils.nvl(txtAmt.getValue(),
					"0")));
			qe.setParaValue("ORD_DISCAMT", df.parse(utils.nvl(txtDiscAmt
					.getValue(), "0")));
		...
		try {
			...
			utilsVaadin.findColByCol("ORD_PRICE", table.listFields).actionAfterUpdate = new afterModelUpdated() {
				public Object onValueChange(int rowno, String colname, Object vl) {
					...
					table.data.setFieldValue(rowno, "ORD_ALLQTY", BigDecimal
							.valueOf(allqt));
					table.data.setFieldValue(rowno, "AMOUNT", BigDecimal
							.valueOf(amt));
					table.data.setFieldValue(rowno, "LSAMT", BigDecimal
							.valueOf(lsamt));

					table.data.setFieldValue(rowno, "ORD_COST_AMT", BigDecimal
							.valueOf(csamt));
			...
			utilsVaadin.findColByCol("AMOUNT", table.listFields).actionAfterUpdate = new afterModelUpdated() {
				public Object onValueChange(int rowno, String colname, Object vl) {
					...
					table.data.setFieldValue(rowno, "ORD_DISCAMT", BigDecimal
							.valueOf(0.0f));
					...
					table.data.setFieldValue(rowno, "ORD_PRICE", BigDecimal
							.valueOf(pr));
			...
			utilsVaadin.findColByCol("ORD_DISCAMT", table.listFields).actionAfterUpdate = new afterModelUpdated() {
				public Object onValueChange(int rowno, String colname, Object vl) {
					...
					if (colname.equals("ORD_DISCAMT")) {
						...
						table.data.setFieldValue(rowno, "DISCP", BigDecimal
								.valueOf(discp));
					}
				...
				public Object onValueChange(int rowno, String colname, Object vl) {
					...
					try {
						...
						if (rst != null && rst.first()) {
							table.data.setFieldValue(rowno, "DESCR", rst
									.getString("DESCR"));
							table.data.setFieldValue(rowno, "ORD_PACKD", rst
									.getString("PACKD"));

							table.data.setFieldValue(rowno, "ORD_UNITD", rst
									.getString("UNITD"));
							...
							table.data.setFieldValue(rowno, "ORD_PACK", rst
									.getBigDecimal("PACK"));

							table.data.setFieldValue(rowno, "LSPRICE", rst
									.getBigDecimal("LSPRICE"));
							table.data.setFieldValue(rowno, "ITPACK", rst
									.getBigDecimal("PACK"));
							table.data.setFieldValue(rowno, "ORD_COST_PRICE",
									rst.getBigDecimal("PACK_COST"));
							...
							utilsVaadin.refreshRowFromData(table.data, table
									.getTable(), rowno, table.listFields);
	...
	public void print_data() {
		...
		try {
			utilsVaadinPrintHandler.printSO(Double.valueOf(QRYSES), "rptVouSO",
					con);
		} catch (Exception ex) {
	...
	public void setupValues() {
		varOrdLocation = Channelplus3Application.getInstance().getFrmUserLogin()
	...
	public void createView() {
		...
		utils.findFieldInfoByObject(txtAmt, lstfldinfo).format = Channelplus3Application.getInstance().getFrmUserLogin().FORMAT_MONEY;
		utils.findFieldInfoByObject(txtAmt, lstfldinfo).fieldType = Parameter.DATA_TYPE_NUMBER;

		utils.findFieldInfoByObject(txtDiscAmt, lstfldinfo).format = Channelplus3Application.getInstance().getFrmUserLogin().FORMAT_MONEY;
		...
		ResourceManager.addComponent(content1Layout, emptyLabel1);
		...
		content1Layout.setExpandRatio(emptyLabel1, 1.6f);
		content1Layout.setExpandRatio(chkFlag, 1.3f);
		content1Layout.setExpandRatio(locationLayout, 1.1f);
		...
		if (!listnerAdded) {
			...
			txtCust.addListener(new ValueChangeListener() {
				...
				public void valueChange(ValueChangeEvent event) {
					...
					utilsVaadin.setValueByForce(txtCustName, txtCust
							.getDisplay());
	...
	protected String get_item_listSQL() {
		...
		if (txtReference.getValue() != null
			...
			if (!typ.equals("1"))
				sq = "select distinct reference,items.descr from items,order2 where order2.ord_code=decode("
						+ typ
						+ ",2,106,3,101) and ord_refer=reference and order2.ord_no='"
						+ txtReference.getValue() + "'";
	...
	public void show_stand_alone(Window wnd, String typ, String txtRefer,
		...
		try {
			...
			if (typ.equals("2")) {
				show_selection_items();
			}
			utilsVaadin.findColByCol("ORD_REFER", table.listFields).searchListSQL = get_item_listSQL();
			txtType.setEnabled(false);
			txtReference.setEnabled(false);
		} catch (Exception e) {
			Channelplus3Application.getInstance().getMainWindow().removeWindow(
					wnd);
			e.printStackTrace();
		}
	...
	private void show_selection_items() throws Exception {
